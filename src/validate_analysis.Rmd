...
```{r}
olap <- intersect(row.names(bmcrpc), final_markers$gene) %>% intersect( gep$GeneSymbol)
...
Heatmap(matrix = mat, right_annotation = row_ha, col = colorRampPalette(c("purple","black", "yellow"))(3), show_column_names = T, cluster_columns = F, na_col = 'black', cluster_rows = F, column_split =as.character(c("A", rep("B", 1),rep("C",1),rep("D", 1), "E" , "F")), row_split = c(rep("A",16 ), rep("B", 2), rep("C", 9), rep("D",11 ), rep("E", 3), rep("F", 4)) )
...
Heatmap(matrix = mat, right_annotation = row_ha, col = colorRampPalette(c("purple","black", "yellow"))(3), show_column_names = T, cluster_columns = F, na_col = 'black', cluster_rows = F, column_split =as.character(c("A", rep("B", 1),rep("C",1),rep("D", 1), "E" , "F")), row_split = c(rep("A",16 ), rep("B", 2), rep("C", 9), rep("D",11 ), rep("E", 3), rep("F", 4)), height = unit(10, 'cm'),row_names_gp = grid::gpar(fontsize = 8) , column_names_gp = gpar(fontsize = 10) )
dev.off()
...
anno_col
pdf("/Users/ds/Desktop/plot.pdf", width = 10, height = 10)
Heatmap(matrix = mat, right_annotation = row_ha, col = colorRampPalette(c("purple","black", "yellow"))(3), show_column_names = T, cluster_columns = F, na_col = 'black', cluster_rows = F, column_split =as.character(c("A", rep("B", 4),rep("C",9),rep("D", 7), "E" )), row_split = c(rep("A",16 ), rep("B", 2), rep("C", 9), rep("D",11 ), rep("E", 3), rep("F", 4)) )
dev.off()
