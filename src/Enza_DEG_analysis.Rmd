...
p
#ggsave(filename = "/Users/ds/Desktop/plot.pdf", width = 10, plot = plot)
```
...
res <- do.call(rbind, lapply(pathways, FUN = function(p){
  ...
  print(p)
  a <- tumor[c(up$Genes),] %>% GetAssayData() %>% colMeans()
  b <- tumor[c(down$Genes),] %>% GetAssayData() %>% colMeans()
  g <- gs %>% dplyr::filter(gs_name == p) %>% .$gene_symbol
  c <- tumor[g,] %>% GetAssayData() %>% colMeans()
...
p1 <- ggviolin(
  ...
  facet.by = "PSA", draw_quantiles = 0.5)
