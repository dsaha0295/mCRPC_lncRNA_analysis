...
write.csv(x = sc_deg, file = "/Users/ds/Desktop/Supplementary.Table4.csv",quote = F, row.names = F)
...
sc_deg %>% dplyr::filter(Genes == "RMST")
```
...
p
dev.off()
mets %>% dplyr::select(c(Beltran.et.al, Ramnarine.et.al, Single.cell,Hedgehog)) %>% gather(key = "Signature" , value = "GEX", -Hedgehog ) %>% ggplot(aes(x = GEX, y = Hedgehog, color = Signature))  %>% facet(facet.by = "Signature") + geom_point() + geom_smooth(method = 'lm') + stat_cor() + scale_color_manual(values = c("navy", 'darkred', 'darkgreen')) + theme_classic()
...
mets %>% gather(key = "Sig", value = "Score", -c(sample_id, Histology, Hedgehog)) %>% ggplot(aes(x = Histology, y = Score, fill = Sig)) + geom_boxplot()
...
DotPlot(object = bmet, features = c("PCA3"))
