...
keep.exprs <- filterByExpr(as.matrix(mcrpc_rna))
...
anno <- anno %>% dplyr::select(c(sample_id, wgs_id, biopsy_site, disease_type_SIG_GEX, ID_patient,tumor_purity_rna )) 
...
saveRDS(object = anno, file = "/Users/ds/Desktop/projects/lncRNA/data//Histo_mCRPC_WGBS_anno.rds")


anno
cf[!duplicated(cf$wgs_id),] %>% na.omit() %>% .$disease_type_SIG_GEX %>% table()
```
...
res <- do.call(rbind, lapply(pathways, FUN = function(p){
  ...
  print(p)
  a <- bmet[c(sc_sig),] %>% GetAssayData() %>% colMeans()
  b <- bmet[c(adeno_sig),] %>% GetAssayData() %>% colMeans()
...
dss <- data.frame(linc, dss[nearest(x =  GRanges(regions), subject = dss),] ) %>% mutate(Sig = ifelse(gene.name %in% colnames(mcrpc_rna)[colnames(mcrpc_rna) %in% sc_sig], "SC signature", ifelse(gene.name %in% colnames(mcrpc_rna)[colnames(mcrpc_rna) %in% adeno_sig], "Adeno signature", "Other")   ))



plot <- dplyr::filter(dss, Sig %in% c("SC signature","Adeno signature")) %>% ggplot(aes(x = Sig, y= Diff, label = gene.name)) + geom_point() + geom_violin(draw_quantiles = 0.5, aes(fill = Sig, alpha = 0.5)) + geom_jitter(width = 0.1) + xlab("Signature") + ylab("Methylation Difference (SC - Adeno)") +   scale_fill_manual(values = c("navy", "darkred")) + theme_classic()
...
```{r}
nepc_lnc <- read_csv(file = "/Users/ds/Desktop/beltran_nepc_lnc.csv", col_names = T, skip = 1)
nepc_lnc <- nepc_lnc %>% dplyr::filter(`NEPC (n=122)` == 1)
#nepc_lnc <- nepc_lnc %>% dplyr::filter(`NEtD (n=742)` == 1)


nepc <- read_csv(file = "/Users/ds/Desktop/nepc_genes.csv",col_names = T)

comb_rna <- mcrpc_rna[,query]

Beltran <- mcrpc_rna[, colnames(mcrpc_rna) %in% nepc$`HGNC ID`] %>% rowMeans()
Ramnarine <- mcrpc_rna[, colnames(mcrpc_rna) %in% nepc_lnc$lncRNA] %>% rowMeans()
Single_cell <- mcrpc_rna[, colnames(mcrpc_rna) %in% c(adeno_sig, sc_sig)] %>% rowMeans()



df <- data.frame(sample_id = names(Beltran), Beltran.et.al = Beltran, Ramnarine.et.al = Ramnarine, Single.cell = Single_cell) %>% merge(anno) 
...
rocobj1 <- roc(labels, predictions)
auc1 <- round(auc(labels, predictions),4)

predictions <- mets$Single.cell

rocobj2 <- roc(labels, predictions)
auc2 <- round(auc(labels, predictions),4)


predictions <- mets$Beltran.et.al

rocobj3 <- roc(labels, predictions)
auc3 <- round(auc(labels, predictions),4)



#create ROC plot
p <- ggroc(list(Ramnarine.et.al = rocobj1, Single.cell = rocobj2, Beltran.et.al = rocobj3), size = 1)+ scale_color_manual(values = c("navy", "darkred", "darkgreen"), name = "Signature", labels = c(paste0("Ramnarine AUC= ", auc1), paste0("Single cell AUC=", auc2), paste0("Beltran AUC=", auc3)) ) + theme_classic() + ggtitle('AUC analysis of 71 CRPC and 3 NEPC')
...
query <- linc[linc$gene.name %in% sc_sig,'gene', drop = T] 
...
paste0(sc_sig, collapse = " ")


sc_sig[sc_sig == "MTCL-AS1"]


data.frame(sc_sig) %>% View()
```
...
nepc <- read_tsv(file = "/Users/ds/Desktop/nepc_dmr_hg38.bed", col_names = c("chr", "start", "end", "Diff"))




nepc <- GRanges(nepc)
nepc <- data.frame(linc, nepc[nearest(x =  GRanges(regions), subject = nepc),] ) %>% mutate(Sig = ifelse(gene.name %in% colnames(mcrpc_rna)[colnames(mcrpc_rna) %in% sc_sig], "Small cell", ifelse(gene.name %in% colnames(mcrpc_rna)[colnames(mcrpc_rna) %in% adeno_sig], "Adeno", "Other")   ))



dplyr::filter(nepc, Sig %in% c("Small cell","Adeno"))  %>% ggplot(aes(x = Sig, y= Diff)) + geom_violin(draw_quantiles = 0.5) + geom_jitter(width = 0.1) 
