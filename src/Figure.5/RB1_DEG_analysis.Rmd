...
df <- t(gsva.es) %>% data.frame(sample_id = row.names(.), TME.pcg.activity = .)
gsva.es <- gsva(t(mcrpc_rna), gset.idx.list = list(c(tumor.down)), method = 'zscore')
df <- t(gsva.es) %>% data.frame(sample_id = row.names(.), Pca.pcg.activity = .) %>% merge(df) %>% merge(anno) %>% inner_join(y = clin, by = c("ID_patient" = "Patient.ID"))
...
df <- df %>% mutate(Group = as.factor(ntile(TME.pcg.activity, 2)))
...
p2 <- coxph(Surv(time =OS.mCRPC , event = Event)~ Pca.pcg.activity, data = df) %>% ggforest(fontsize = 2)
...
p4 <- coxph(Surv(time =OS.mCRPC , event = Event)~ Pca.pcg.activity  +ECOG.PS + PSA.At.Biopsy+ LDH.at.biopsy + ALP + Hemoglobin.at.biopsy + Disease.Site + Enzalutamide.resistant  , data = df) %>% ggforest(fontsize = 2)
