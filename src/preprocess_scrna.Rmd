...
FeaturePlot(object = bmet, features =c("PCAT14", "MIAT"), label = T, repel = T, label.size = 3)
dev.off()
final_markers

UMAPPlot(object = tumor, label = T, repel = T)
#tumor$Androgen.response <- tumor[tni.get(object = rtni,what = "regulons" )$AR,] %>% GetAssayData() %>% colMeans()
